...
function monthlyChartAJAX(graphType, sellYear, clothType){
	...
	//ajax
	$.ajax({
		 url : "/joeunmall/graphJsonVO/monthlyGraphData",
		 type : 'get',
		 dataType:'text',
		 data : {
		 ...
		 success : function(msg) {
			...
			console.log("msg의 길이: " + msg.length);		
			//controller에서 받는 data정보 
			if(msg.length != 2){
				...
				console.log("json 2번째 period 앞 두자리 뺀 값: " + parseInt(json[1].period.slice(2)));
				console.log("json 1번째 period 앞 두자리 뺀 값: " + parseInt(json[0].period.slice(2)));
				
				for(var i=0; i<json.length; i++){
					if(100 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 131){
						JanList.push(json[i].price);
					} else if(200 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 231){
						FebList.push(json[i].price);
					} else if(300 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 331){
						MarList.push(json[i].price);
					} else if(400 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 431){
						AprList.push(json[i].price);
					} else if(500 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 531){
						MayList.push(json[i].price)
					} else if(600 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 631){
						JunList.push(json[i].price);
					} else if(700 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 731){
						JulList.push(json[i].price);
					} else if(800 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 831){
						AugList.push(json[i].price);
					} else if(900 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 931){
						SepList.push(json[i].price);
					} else if(1000 < parseInt(json[i].period.slice(2)) && parseInt(json[i].period.slice(2)) < 1031){
